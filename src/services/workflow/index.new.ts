...
// Import Ollama service for backward compatibility
import { generateWorkflowWithOllama } from '../ai/ollama';

// Import new workflow orchestrator
import { WorkflowOrchestrator, WorkflowResult } from '../../application/storytelling/services/WorkflowOrchestrator';
 ...
 * 
 * This maintains backward compatibility with the existing API while
...
export async function generateEpisodeWorkflow(
  ...
  try {
    // Use the new workflow orchestrator for advanced story generation
    return await workflowOrchestrator.generateEpisodeWorkflow(
      config, 
      stylePresets,
      characters
    );
  } catch (error) {
    console.error('Enhanced workflow generation failed, falling back to legacy approach:', error);
    return await legacyGenerateEpisodeWorkflow(config, stylePresets, characters);
  }
}
 ...
 */
async function legacyGenerateEpisodeWorkflow(
  config: EpisodeWorkflowConfig,
  stylePresets: StylePreset[],
  characters: Character[]
): Promise<WorkflowResult> {
  try {
    console.log('Attempting to generate with Ollama...');
    ...
    console.log('Successfully generated workflow with Ollama');
 ...
 * This preserves the original fallback functionality
 */
function generateLocalWorkflow(
  ...
  console.log('Using local workflow generation');
...
// Helper functions preserved from the original implementation
