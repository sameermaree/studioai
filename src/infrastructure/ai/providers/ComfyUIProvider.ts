...
export class ComfyUIProvider implements AIProvider {
  ...
  async generateImage(
    ...
    if (options?.workflowPath && 
      ...
      // This looks like a fallback dynamic workflow (only nodes 3-9, generated by buildPromptWorkflow)
      ...
      if (isMissingFile) {
        ...
        console.error('[WORKFLOW ERROR] The fallback buildPromptWorkflow() generates generic settings — not the Disney/Pixar workflow.');
        ...
        throw new Error(
          ...
          `Fallback dynamic build was used but is forbidden — only the Disney/Pixar workflow file is valid. ` +
          `Checkpoint node value: "${checkpointFromNode}". ` +
          `Expected workflow: workflows/pixar_disney_stable.json with DreamShaper checkpoint.`
        );
      }
    }
    console.log('=====================================================');
